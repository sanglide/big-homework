...
public class ScheduleServiceImpl implements ScheduleService, ScheduleServiceForBl {
	...
	public ResponseVO searchAudienceSchedule(int movieId) {
		try {
			...
			int interval = 1;
			if (result.size() > 0) {
				interval = (int) ((result.get(result.size() - 1).getStartTime().getTime() - today.getTime())
						/ (1000 * 3600 * 24)) + 1;
			}
	...
	ResponseVO preCheck(ScheduleForm scheduleForm) {
		try {
			...
			// 处理排片跨天错误
			if (!simpleDateFormat.format(scheduleForm.getStartTime())
					.equals(simpleDateFormat.format(scheduleForm.getEndTime()))) {
			...
			if (0 != scheduleMapper.selectScheduleConflictByHallIdAndTime(scheduleForm.getHallId(),
					scheduleForm.getStartTime(), scheduleForm.getEndTime(), id).size()) {
	...
	List<ScheduleVO> getScheduleVOList(int interval, Date startDate, List<ScheduleItem> scheduleItemList)
